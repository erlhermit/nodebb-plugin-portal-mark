 ...
 define('admin/plugins/portalmark', ['forum/infinitescroll', 'admin/modules/selectable'], function(infinitescroll, selectable) {
	...
	function handleModify() {
		...
		$('#modify').on('click', function(ev) {
			...
			modelselector.empty();
			tags.push({tag:'',name:'---NO PARENT---'});
			for(var i = 0; i < marksTags.length; i++){
				var mark = $(marksTags[i]);
				if(true != mark.hasClass('selected')){
					tags.push({tag:mark.find('[data-name="mark-tag"]').val(),name:mark.find('[data-name="mark-name"]').val()});
				}
			}
	...
	function handleDeleteSelected() {
		$('#deleteSelected').on('click', function() {
			...
			bootbox.confirm('Do you want to delete the selected marks? also delete from titled topics.', function(confirm) {
				...
				socket.emit('admin.plugins.'+socketId+'.removeMarks', {marks: marks}, function(err) {
					if (err) {
	 ...
	 function handleSearch() {
		 $('#tag-search').on('input propertychange', function() {
			 ...
			 timeoutId = setTimeout(function() {
				 socket.emit('topics.searchAndLoadmarks', {query: $('#mark-search').val()}, function(err, marks) {
					 if (err) {
						 return app.alertError(err.message);
					 }

					 infinitescroll.parseAndTranslate('admin/manage/portalmark', 'marks', {marks: marks}, function(html) {
						 $('.mark-list').html(html);
